...
var $dp, WdatePicker;
(function () {
    var themeName = 'wi-all.css',// 组件 css
        jsName = 'WdatePicker.js';// 与 calendar.js 同路径的 js 文件名
    var $ = {
        $langList: [
    ...
    WdatePicker = U;
    var Y = window, T = {innerHTML: ""}, N = "document", H = "documentElement", C = "getElementsByTagName", V, S, G, c, X = navigator.appName;
    if (X == "Microsoft Internet Explorer")S = true; else if (X == "Opera")c = true; else G = true;
    var themeSrc = getCssSrc(themeName) || '',//add by qq 根据 WebUI4Angular-tpls-all.js 获取组件根目录路径
        jsSrc = $.$dpPath || getJsSrc(jsName);//tab by qq 获取 js 路径
//    if ($.$wdate)K(A + "skin/WdatePicker.css"); --del by qq
    V = Y;
    if ($.$crossFrame) {
        try {
    ...
    function getCssSrc(css){// add by qq 获取弹出页面（源） wi-css 引入路径，设置 iframe 的 css 路径
        var _, $ = Y[N][C]('link');// document.getElementsByTagName
        for (var B = 0; B < $.length; B++) {
            _ = $[B].getAttribute("href") || "";
            if(_.indexOf(css)>=0) break;// 文件名
            _ = '';
        }
        return _
    }
    function getJsSrc(js) {// add by qq 获取弹出页面（源） js 引入路径，设置 iframe 的 css 路径
        var _, A, $ = Y[N][C]('script');// document.getElementsByTagName
        for (var B = 0; B < $.length; B++) {
            _ = $[B].getAttribute("src") || "";
            _ = _.substr(0, _.indexOf(js));
            A = _.lastIndexOf("/");
    ...
    function I(G, _) {
        ...
        function E(K, J) {
            ...
            function C() {
                ...
                $.push("<link rel=\"stylesheet\" type=\"text/css\" href=\"" + themeSrc + "\" charset=\"" + D[0].charset + "\"/>");
                $.push("<script src=\"" + jsSrc + "calendar.js\"></script>");
